...
import time 
# 权重下载地址: https://pan.baidu.com/s/1BwF4ermPvj8ioAOqGjtAGA 提取码: ivzg 复制这段内容后打开百度网盘手机App，操作更方便哦
weight_save_path = os.path.join(model_path, 'kesicmethod7_submit.weights') # kesicnl2sql_finetune.weight is good 
token_dict = {}
...
if mode == 'train':
    ...
    datas = np.load('kesic_test_new_method_samp12_of_2250_len380.npy', allow_pickle=True).tolist()
    test_data = [] 
    for d in datas:
        test_data.append((d['para'], d['question'], d['question_id']))
...
def test(test_data):
    ...
    for val in test_data:
        ...
        # print(q_id)
        q_id_ans_dict.setdefault(q_id, {'q_cnt': {}, 'answer': {}})
        ...
        if 1 in startpos[:len(passage)] and 1 in endpos[:len(passage)]:
            ...
            q_id_ans_dict[q_id]['answer'].setdefault(ans, 0)
            q_id_ans_dict[q_id]['answer'][ans] += 1 
            q_id_ans_dict[q_id]['q_cnt'].setdefault(p_ans_cnt, 0)
            q_id_ans_dict[q_id]['q_cnt'][p_ans_cnt] += 1 
            # print(ans)
    ...
    np.save('./test_res.npy', q_id_ans_dict)
    sys.exit(0)
    import  time  
