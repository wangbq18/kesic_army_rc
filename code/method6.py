...
import time 
weight_save_path = os.path.join(model_path, 'kesicnl2sql_finetune_6.weights') # kesicnl2sql_finetune.weight is good 
token_dict = {}
...
if mode == 'train':
    datas = np.load('kesic_508.npy', allow_pickle=True).tolist()
    data = [] 
    for q in datas:
        d = datas[q]
        # print('*' * 10)
    ...
    def __iter__(self):
        while True:
            ...
            for i in idxs:
                ...
                #     # assert ans_end_pos.tolist().count(1) == 2 
                #     # assert ans_start_pos.tolist().count(1) == 2 
...
p = Dense(10, activation='softmax')(q_cls)
...
# learning_rate = 5e-5
learning_rate = 6e-6# from 15 to 8 
