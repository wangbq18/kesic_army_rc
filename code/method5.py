...
weight_save_path = os.path.join(model_path, 'kesicnl2sql_finetune_method5.weights') # kesicnl2sql_finetune.weight is good 
token_dict = {}
...
if mode == 'train':
    datas = np.load('kesic_320.npy', allow_pickle=True).tolist()
    data = [] 
    for q in datas:
        d = datas[q]
        #print(d)
    ...
    datas = np.load('kesic_test320.npy', allow_pickle=True).tolist()
    test_data = [] 
    for d in datas:
...
def test(test_data):
    ...
    write_csv(submit, './testsubmitmethod5.csv')
