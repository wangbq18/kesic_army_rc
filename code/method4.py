...
weight_save_path = os.path.join(model_path, 'kesicnl2sql_finetune_method4.weights') # kesicnl2sql_finetune.weight is good 
token_dict = {}
...
def test(test_data):
    ...
    for val in test_data:
        ...
        p_ans_cnt, ans_start, ans_end = model.predict([np.array([x1]), np.array([x2]), np.array([h]),  np.array([passage_mask])])
        ...
        if 1 in startpos and 1 in endpos:
            ...
            if len(ans) == 0:
                ...
                print(question)
                print('*' * 10)
                start_idx = startpos.index(1) - 1
                end_idx  = endpos.index(1)
                print(start_idx, end_idx)
                print(passage[start_idx: start_idx + 10])
                
                print(passage[end_idx - 10: end_idx])
            cnt += 1 
        else:
            pass 
            # print('-' * 10)
